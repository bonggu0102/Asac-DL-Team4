...
실측 천장고 2.3m를 기준으로 공간 스케일을 보정했습니다. 발표 자료의 두 측정 사례는 2.25m와 2.02m로, 절대 오차는 각각 0.05m와 0.28m였습니다.
...
- 노트북 20개 JSON 로드 확인
- 코드 셀 361개 정적 Python 구문 검사 통과
- 저장된 오류 출력 없음
- 기존 개인 경로(`Othercomputers`, 이전 영상 파일명, `gdown --folder`) 참조 제거
- 코드가 요구하는 주요 폴더·영상·가중치·마스크·가구 파일명을 Google Drive와 대조

전체 GPU 파이프라인은 공개용 정리 후 다시 일괄 실행하지 않았습니다. 라이브러리 버전이나 Colab 런타임에 따라 설치 명령 또는 모델 API의 추가 조정이 필요할 수 있습니다.
